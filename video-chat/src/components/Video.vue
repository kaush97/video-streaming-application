<template>
  <div class="col-md-5 box">
    <div class="roomTitle">
      <span v-if="loading"
        ><h4>Loading... {{ roomName }}</h4></span
      >
      <span v-else-if="!loading && roomName"
        ><h4>Connected to {{ roomName }}</h4></span
      >
      <span v-else><h4>Select a room to get started</h4></span>
    </div>
      ...
      this.getAccessToken().then((data) => {
        ...
        Twilio.connect(token, connectOptions).then(function (room) {
          ...
          room.on("participantConnected", function (participant) {
            ...
            document.getElementById("localTrack").appendChild(div);
          });
...
.roomTitle {
  ...
  color: #cef0d4;
   box-sizing: border-box;
     box-shadow: 0 5px 20px rgba(249, 250, 250, 0.5);
  border-radius: 10px;
}
</style>
